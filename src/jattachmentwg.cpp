...
{ 
    ...
    Trans trans;
    QString fileName = ATTACHPATH + m_fileBt->text();

    if (!trans.isAttachmentUsed(m_fileBt->text()) && QFile::exists(fileName)) {
        if (!QFile::remove(fileName))
        message(J_ERROR, "Attachment", "Failed to delete the attached file. Please remove " + fileName + "manually");
    }
    // remove it from screen and db
    m_fileBt->setText("");
    m_fileBt->setEnabled(false);
  ...
  //  qDebug() << "delete attached file!" << fileName;
}
